...
export const List = ({ title, listId, parentId }) => {
  ...
  const cards = data?.allCards.map((card) => {
    return (
      <Card key={card._id} id={card._id} listId={listId} label={card.title} />
    );
  });
  ...
  return (
    <>
      <motion.div
        ...
        onDrop={drop}
        onDragOver={dragOver}
        className='mr-3 grid auto-cols-auto grid-rows-1 gap-4 justify-center items-start bg-gray-200 rounded-lg overflow-hidden overflow-y-scroll w-72 height-min p-3 z-50'
      >
